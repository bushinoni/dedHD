...
(function () {
    ...
    const resetAppToNormal = () => {
        document.body.className = document.body.className.replace(/\bff-[^\s]+/g, '').replace(/\blv[123]\b/g, '');
        const els = ['ff-prompt', 'ff-revert-btn', 'ff-focus-spotlight', 'ff-breathing', 'ff-break-overlay', 'ff-toast'];
        els.forEach(id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
        const edge = document.getElementById('ff-edge-glow');
        if (edge) edge.style.boxShadow = 'none';
        
        masterLevel = 0;
        simplificationAllowed = null;
    };
    ...
    window.addEventListener('focus', () => {
        ...
        raw.mousePositions.push({ x:0, y:0, t: now });
        raw.lastInputActivity = now;
    ...
    const computeSignals = () => {
        ...
        signals.idle = idleSec > 120 ? 3 : idleSec > 60 ? 2 : idleSec > 30 ? 1 : 0;
        ...
        const avgSpeed = recentScrolls.length
            ...
            : 0;
        signals.scrollSpeed = avgSpeed > 10 ? 3 : avgSpeed > 5.5 ? 2 : avgSpeed > 2.5 ? 1 : 0;
